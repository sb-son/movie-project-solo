...
const editModalContent = `<div class="modal fade" id="editModalContent" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
    ...
    fetch(`https://adaptive-dent-wasabi.glitch.me/movies`)
        ...
        .then((data) => {
            ...
            data.forEach(function (m) {
                ...
                $("#movies").append(`<div class="card d-flex col-4 justify-content-between p-0"><div id="front" class="cardFront">Title: ${m.title} Rating: ${m.rating} <span id="movie-id" style="display: none">${m.id}</span><div>${editBtn}<button type="button" class="flip-btn" id="flip-btn">Flip Card</button></div></div><div id="back" class="cardBack">${deleteBtn} ${cancelBtn}</div> ${editModalContent}</div>`);
            })
